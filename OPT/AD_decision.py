...
class Opt(object):
    def __init__(self, args):
        ...
        if args['policy'] == 'opt_t_const':
            self.t_const = True
        else: self.t_const = False
        ...
        self.coef_traveling_t = args['coef_traveling_t']
    ...
    def run_opt(self, args, cur_t, sim_result, saa_sample_sce=False, outputflag=False):
        if self.saa:
            self.pat_amb = deepcopy(args['s_pat_amb'][saa_sample_sce])
            self.pat_walk = deepcopy(args['s_pat_walk'][saa_sample_sce])
        ...
        if self.saa:
            m.setObjective((self.alpha * quicksum(obj1[l, i, t] for l in Ls for i in Hs for t in Ts if t >= self.thd[l] and t >= cur_t)
                            - self.beta * quicksum(mu[l, i, t] for l in Ls for i in Hs for t in Ts if t >= cur_t)
                            + self.gamma * quicksum(1 - x[i, t] for i in Hs for t in Ts if t >= cur_t)
                            + self.delta * quicksum(1 - y[i, t] for i in Hs for t in Ts if t >= cur_t)
                            + self.coef_traveling_t *
                            quicksum(self.traveling_t[i][j] * f[l, i, j, t] * len(self.pat_amb[(l, j, t)])
            ...
            m.setObjective((self.alpha * quicksum(obj1[l, i, t] for l in Ls for i in Hs for t in Ts if t >= self.thd[l] and t >= cur_t)
                            ...
                            + self.delta * quicksum(1 - y[i, t] for i in Hs for t in Ts if t >= cur_t)
                            + self.coef_traveling_t *
                            quicksum(self.traveling_t[i][j] * f[l, i, j, t] * self.d_amb[l][i][self.t2day(t)]
                                     for l in Ls for i in Hs for j in Hs for t in Ts if t >= cur_t)
                            ), GRB.MINIMIZE)
        ...
        if self.saa:
            m.addConstrs((obj1[l, i, t] >=
                          quicksum(quicksum(f[l, j, i, t2] * len(self.pat_amb[(l, j, t2)]) for j in Hs) + len(self.pat_walk[(l, i, t2)])
        ...
        # # AD 지속시간 체크: AD 중인데 지속기간이 opt_open을 못채웠으면 계속 AD 시킴
        if self.t_const:
            m.addConstrs(
                (x[i, t] <= x[i, t - 1] + 1 - quicksum(x[i, t2] for t2 in Ts if t - args['opt_open_urg'] <= t2 <= t - 1) / args['opt_open_urg']
                 for i in Hs for t in Ts if t >= args['opt_open_urg'] and t >= cur_t), name='t_const1')
            m.addConstrs((y[i, t] <= y[i, t - 1] + 1 - quicksum(y[i, t2] for t2 in Ts if t - args['opt_open_nonurg'] <= t2 <= t - 1) / args[
                'opt_open_nonurg'] for i in Hs for t in Ts if t >= args['opt_open_nonurg'] and t >= cur_t), name='t_const2')
            m.addConstrs((x[i, t] <= x[i, t - 1] for i in Hs for t in Ts if cur_t <= t < args['opt_open_urg'] and t >= 1), name='t_const3')
            m.addConstrs((y[i, t] <= y[i, t - 1] for i in Hs for t in Ts if cur_t <= t < args['opt_open_nonurg'] and t >= 1), name='t_const4')
        ...
        if self.saa:
            m.addConstrs((
                quicksum(f[l, j, i, t - self.dist[j][i]] * len(self.pat_amb[(l, j, t - self.dist[j][i])]) for j in Hs
                         if t - self.dist[i][j] >= cur_t)
                + quicksum(f[l, j, i, t - self.dist[j][i]] * sim_result['pat_amb_hist'][l][j][t - self.dist[j][i]] for j in Hs
                           if t - self.dist[i][j] < cur_t)
                + len(self.pat_walk[(l, i, t)]) == lamda[l, i, t]
                for l in Ls for i in Hs for t in Ts if t >= cur_t and t >= np.max(self.dist)), name='Constr15')
            m.addConstrs((len(self.pat_walk[(l, i, t)]) == lamda[l, i, t] for l in Ls for i in Hs for t in Ts if
                          cur_t <= t < np.max(self.dist)), name='Constr16')
        else:
            m.addConstrs((
        ...
        """ 
        # 한 번에 OPEN <->A-AD로 바뀌는 것 방지. L-AD를 단계별로 거쳐야 함.
        m.addConstrs(((x[i, t1] + y[i, t1]) - (x[i, t1 + 1] + y[i, t1 + 1]) <= 1 for i in Hs for t1 in Ts
                      if t1 <= current_t + Time_planning - 2), name='Constr13')
        m.addConstrs(((x[i, t1] + y[i, t1]) - (x[i, t1 + 1] + y[i, t1 + 1]) >= -1 for i in Hs for t1 in Ts
                      if t1 <= current_t + Time_planning - 2), name='Constr14')
        """
            ...
            if self.saa:
                ...
                for i in Hs:
                    for tp in range(self.tp):
                        AD_DV_temp["x, %d, %d" % (i, cur_t + tp)] = round(x[i, cur_t + tp].x)
                        AD_DV_temp["y, %d, %d" % (i, cur_t + tp)] = round(y[i, cur_t + tp].x)
                return AD_DV_temp
            else:
                AD_result_test = {}
                for v in m.getVars():
